...
double MGI_Parameters_Calculator::differ(double x, double y)
{
	...
	for(s=h=pow(2+x,-alpha),n=3; h/s>1.e-6; n++) s+=h=pow(n+x,-alpha);//EARLY WAS h/s>1.e-6	это повлияло на минимум графика	!!!!!!!!!
	imax=n;
	s2=s;

	for(ss[1]=s, i=2; i<imax; i++) {//EARLY WAS  i<=IMAX && i<imax		это повлияло на минимум графика			!!!!!!!!!
		s-=pow(i+x,-alpha);
