...
class Traffic_Stats : public cSimpleModule
{
  ...
  protected:
    ...
    // Fields for packet size histogram of non discretizated traffic											NEW!!!
    TrafficHistogram *sizeHistogram;
    int numHistCellsForTraffic; // for packet size histogram of non discretizated traffic 
    int allowedMaxTrafficValue; // maximal allowed traffic value, which will be added to histogram
...
void Traffic_Stats::initialize()
{
    ...
    // For packet size histogram calculation																	NEW!!!
    numHistCellsForTraffic = par("numHistCellsForTraffic");
    allowedMaxTrafficValue = par("allowedMaxTrafficValue");
    sizeHistogram = new TrafficHistogram(numHistCellsForTraffic, allowedMaxTrafficValue);
...
void Traffic_Stats::handleMessage(cMessage *msg)
{
	...
	sizeHistogram->putInHist(pkt->getByteLength());            														//NEW!!!
...
void Traffic_Stats::finish()
{
    ...
    std::ofstream out( helper->createFileName(outFileName, par("numberOfExperiment_"), par("outFileName").stringValue(), (int)par("fileSuffix")) );
    ...
    out<<"  <MBPS> "<< (dst.getMean() * cellsize / timeslot) * 8 / 1024 / 1024<<" </MBPS>"<<endl; //for comparing with datarate of channel
    out<<" </TRAFFIC-MEAN>"<<endl;
    ...
    out<<" <SIZE-HISTOGRAM>"<<endl;																						//NEW!!!
    out<<"  <ALLOWED-MAX-TRAFFIC-VALUE> "<< sizeHistogram->getAllowedMaxTrafficValue() <<" </ALLOWED-MAX-TRAFFIC-VALUE>"<<endl;
    out<<"  <NUM-HIST-CELLS-FOR-TRAFFIC> "<< sizeHistogram->getNumHistCells() <<" </NUM-HIST-CELLS-FOR-TRAFFIC>"<<endl;
    out<<"  <WIDTH-OF-CELL> "<< sizeHistogram->getWidthOfCell() <<" </WIDTH-OF-CELL>"<<endl;
    out<<"  <HIST-POINTS>";
    for (i = 0; i < sizeHistogram->getNumHistCells() ; i++){
    	out << sizeHistogram->getHistValue(i) <<"  ";
    }
    out<<"</HIST-POINTS>"<<endl;
    out<<"  <CHECK-SUM> "<<  sizeHistogram->calculateCheckSum() << " </CHECK-SUM>"<<endl;
    out<<"  <OUT-OF-RANGE-PACKETS> "
            << sizeHistogram->getOverflowPackets() + sizeHistogram->getUnderflowPackets() <<" </OUT-OF-RANGE-PACKETS>"<<endl;
    out<<" </SIZE-HISTOGRAM>"<<endl;
