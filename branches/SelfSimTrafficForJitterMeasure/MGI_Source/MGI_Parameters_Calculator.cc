...
void MGI_Parameters_Calculator::initialize()
{
    ...
    f[1]= differ(X[1], Y[1]);
    f[2]= differ(X[2], Y[2]);
    tol = amoeba2D(f, X, Y, 1.e-8, &count );//EARLY WAS 1.0e-8										!!!!!!!!
    double px=X[0], py=Y[0];
    
    int i;
    double s, xx;    
    for(s=py,i=2; (xx=pow(px+i,-alpha))/s > 1.0e-9; s+=xx,i++);//EARLY WAS 1.0e-9					!!!!!!!!
    int imax=i; 
    double A=(1-py)/(s-py);
...
/* --------------------------------------------------------------- */
#define TINY 1.0e-10  /* A small number. */    //early was 1.0e-10											!!!!!!!!
#define NMAX 50000		//early was 5000																	!!!!!!!!
...
#define FIT_POINTS 5	//early was 4																		!!!!!!!!!
#define IMAX 100
...
double MGI_Parameters_Calculator::differ(double x, double y)
{
	...
	for(s=h=pow(2+x,-alpha),n=3; h/s>1.e-6; n++) s+=h=pow(n+x,-alpha);//EARLY WAS h/s>1.e-6	it affected the minimum of graphic	!!!!!!!!!
	imax=n;
	s2=s;

	for(ss[1]=s, i=2; i<imax; i++) {//EARLY WAS  i<=IMAX && i<imax		it affected the minimum of graphic			!!!!!!!!!
		s-=pow(i+x,-alpha);
		for(k=1; k<=FIT_POINTS; k++) if(i>=k) ss[k]+=s;//EARLY WAS if(i>k) ss[k]+=s								!!!!!!!!!
	}  
